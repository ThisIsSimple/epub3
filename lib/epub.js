...
const unzip = require("unzipit").unzip;
...
class EPub extends events_1.EventEmitter {
      ...
      if (this.filename.includes("http")) {
        ...
        this.zip = {
          ...
          readFile: (name, cb) => {
            zip.entries[name]
              .arrayBuffer()
              .then((buffer) => cb(null, new Buffer(buffer)));
          },
          count: zip.entries.length,
        };
        console.log(this.zip.names);
      } else {
        // @ts-ignore
  ...
  getRootFiles() {
    ...
    this.zip.readFile(this.containerFile, (err, data) => {
      ...
      let xml = data.toString("utf-8").toLowerCase().trim(),
        xmlparser = new xml2js_1.default.Parser(xml2jsOptions);
    ...
    for (i = 0, len = keys.length; i < len; i++) {
      ...
      switch (key) {
        case "publisher":
          if (Array.isArray(currentData)) {
            this.metadata.publisher = String(
              (currentData[0] && currentData[0]["#"]) || currentData[0] || ""
            ).trim();
          } else {
            this.metadata.publisher = String(
        ...
        case "subject":
          ...
          (Array.isArray(currentData) ? currentData : [currentData]).forEach(
            function (value) {
        ...
        case "creator":
          if (Array.isArray(currentData)) {
            ...
            this.metadata.creatorFileAs = String(
              (currentData["@"] && currentData["@"]["opf:file-as"]) ||
                this.metadata.creator
            ).trim();
          }
          break;
        ...
        case "meta":
          if (
            currentData["#"] &&
            currentData["@"].property == "calibre:author_link_map"
          ) {
            this.metadata["contribute"] = this.metadata["contribute"] || [];
            this.metadata["author_link_map"] =
              this.metadata["author_link_map"] || {};
            let t = JSON.parse(currentData["#"]);
            for (let n in t) {
              n = n.toString().trim();
              this.metadata["contribute"].push(n);
              this.metadata["author_link_map"][n] = (t[n] || "")
                .toString()
                .trim();
            }
            this.metadata["contribute"] = (0,
            array_hyper_unique_1.array_unique)(this.metadata["contribute"]);
          }
          break;
        default:
      ...
      if (meta["@"] && meta["@"].name) {
        ...
        if (name == "calibre:series") {
          this.metadata["series"] =
            this.metadata["series"] || meta["@"].content;
        }
      }
  ...
  getChapter(chapterId, callback) {
    ...
    this.getChapterRaw(chapterId, (err, str) => {
      ...
      str = str.replace(/<style[^>]*?>(.*?)<\/style[^>]*?>/gi, function (o, s) {
        return "";
      });
  ...
  getChapterRaw(chapterId, callback) {
    if (this.manifest[chapterId]) {
      if (
        !(
          this.manifest[chapterId]["media-type"] == "application/xhtml+xml" ||
          this.manifest[chapterId]["media-type"] == "image/svg+xml"
        )
      ) {
        return callback(
          new Error(
            `Invalid mime type for chapter "${chapterId}" ${this.manifest[chapterId]["media-type"]}`
          )
        );
      }
      this.zip.readFile(
        this.manifest[chapterId].href,
        function (err, data) {
          if (err) {
            callback(
              new Error(
                `Reading archive failed "${chapterId}", ${this.manifest[chapterId].href}`
              )
            );
            return;
          } else if (!data) {
  ...
  getFile(id, callback) {
    if (this.manifest[id]) {
      ...
      this.zip.readFile(this.manifest[id].href, (err, data) => {
        if (err) {
          callback(
            new Error(`Reading archive failed ${self.manifest[id].href}`)
          );
          return;
        }
